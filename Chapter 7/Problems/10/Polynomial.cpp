...
Polynomial derivative(const Polynomial& p) {
  ...
  for (std::list<Polynomial::Term>::const_iterator it = p.polynomial.begin();
       it != p.polynomial.end(); ++it) {
    result += derivative(*it);
  }
  result.polynomial.sort(compare_terms);
  return result;
}
...
Polynomial::Term derivative(const Polynomial::Term& t) {
  ...
  return Polynomial::Term(t.exp * t.coeff, t.exp - 1);
}

Polynomial::Term antiderivative(const Polynomial::Term& t) {
  try {
    if (t.exp == -1) {
      throw "Not a polynomial but the natural logarithm.";
    }
    return Polynomial::Term(t.coeff / (t.exp + 1), t.exp + 1);
  }
...
const Polynomial Polynomial::Term::ONE(1.0, 0);

const Polynomial Polynomial::Term::X(1.0, 1);

const std::list<Polynomial::Term>& Polynomial::get_polynomial() const {
  return polynomial;
}
